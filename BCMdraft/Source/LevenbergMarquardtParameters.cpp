...
PosteriorLevenbergMarquardt::PosteriorLevenbergMarquardt(
    ABC_function* f,
...
PosteriorLevenbergMarquardt::PosteriorLevenbergMarquardt (const PosteriorLevenbergMarquardt& other):
    data_(other.data_),
    w_(other.w_),
...
PosteriorLevenbergMarquardt::operator=(const PosteriorLevenbergMarquardt& other)
{
    if (this!=&other)
    {
        PosteriorLevenbergMarquardt tmp(other);
        swap(*this,tmp);
    }
    return *this;
}

void swap(PosteriorLevenbergMarquardt& one, PosteriorLevenbergMarquardt& other)
{
...
double PosteriorLevenbergMarquardt::getEvidence()const
{

return evidence_;
}
double PosteriorLevenbergMarquardt::getLogPostLik()const
{

return LogPostLik_;
}

double PosteriorLevenbergMarquardt::logDetPriorCov()const
{
    return logDetPriorCov_;
}
double PosteriorLevenbergMarquardt::logDetPostCov()const
{
    return logDetPostCov_;
}

double PosteriorLevenbergMarquardt::logDetPostStd()const
{
    return logDetPostStd_;
}

double PosteriorLevenbergMarquardt::SSdata()const
{
    return SSdata_;
}


void PosteriorLevenbergMarquardt::calculateCovariance()
{

}

void PosteriorLevenbergMarquardt::iterate()
{
    computeJacobian();
...
void PosteriorLevenbergMarquardt::computeJacobian()
{
    ...
    for (std::size_t i=0; i<nPar_; i++)
    {
        ...
        x[i]+=dx_;
        std::vector<double> yfit=m_->yfit(x);
        //std::cout<<nFeval_++<<" ";
        for (std::size_t n=0;n<nData_;++n)
        {
...
void PosteriorLevenbergMarquardt::computeSearchDirection()
{
    for (std::size_t i=0; i<nPar_; ++i)
        for (std::size_t j=0; j<nPar_; ++j)
        {
...
void PosteriorLevenbergMarquardt::updateLanda()
{
...
bool PosteriorLevenbergMarquardt::meetConvergenceCriteria()
{
    surpassIter_=(bool(nIter_>=maxIter_)&&(currSS_>100))||(nIter_>=maxIter_*1);
...
void PosteriorLevenbergMarquardt::initialize()
{
    nIter_=0;
    nFeval_=0;
    ParamChange_=std::numeric_limits<double>::infinity();
...
Parameters PosteriorLevenbergMarquardt::OptimParameters()const
{
    return optimParam_;
}
